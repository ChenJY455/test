...
int main(int argc, char* argv[]){
    ...
    // sort
    if(rank % 2 == 1){  //1, 3, 5, 7
        MPI_Send(maxDisSumPivots, M * k, MPI_INT, rank-1, 0, MPI_COMM_WORLD);
        MPI_Send(minDisSumPivots, M * k, MPI_INT, rank-1, 1, MPI_COMM_WORLD);
        MPI_Send(maxDistanceSum, M, MPI_DOUBLE, rank-1, 2, MPI_COMM_WORLD);
        MPI_Send(minDistanceSum, M, MPI_DOUBLE, rank-1, 3, MPI_COMM_WORLD);
    }
    else{  //0, 2, 4, 6
        MPI_Recv(maxDisSumPivots1, M * k, MPI_INT, rank+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(minDisSumPivots1, M * k, MPI_INT, rank+1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(maxDistanceSum1, M, MPI_DOUBLE, rank+1, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(minDistanceSum1, M, MPI_DOUBLE, rank+1, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    if( rank==2 || rank==6 ){
        MPI_Send(maxDisSumPivots2, M * k, MPI_INT, rank-2, 0, MPI_COMM_WORLD);
        MPI_Send(minDisSumPivots2, M * k, MPI_INT, rank-2, 1, MPI_COMM_WORLD);
        MPI_Send(maxDistanceSum2, M, MPI_DOUBLE, rank-2, 2, MPI_COMM_WORLD);
        MPI_Send(minDistanceSum2, M, MPI_DOUBLE, rank-2, 3, MPI_COMM_WORLD);
    }
    else if( rank==0 || rank==4 ){   //0, 4
        MPI_Recv(maxDisSumPivots, M * k, MPI_INT, rank+2, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(minDisSumPivots, M * k, MPI_INT, rank+2, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(maxDistanceSum, M, MPI_DOUBLE, rank+2, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(minDistanceSum, M, MPI_DOUBLE, rank+2, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        MergeSort(M, k, maxDistanceSum, maxDistanceSum2, maxDistanceSum1, 
            maxDisSumPivots, maxDisSumPivots2, maxDisSumPivots1, MAX);
        MergeSort(M, k, minDistanceSum, minDistanceSum2, minDistanceSum1, 
            minDisSumPivots, minDisSumPivots2, minDisSumPivots1, MIN);
    }

    if(rank==4){
        MPI_Send(maxDisSumPivots1, M * k, MPI_INT, 0, 0, MPI_COMM_WORLD);
        MPI_Send(minDisSumPivots1, M * k, MPI_INT, 0, 1, MPI_COMM_WORLD);
        MPI_Send(maxDistanceSum1, M, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD);
        MPI_Send(minDistanceSum1, M, MPI_DOUBLE, 0, 3, MPI_COMM_WORLD);
    }
    else if(rank==0){
        MPI_Recv(maxDisSumPivots2, M * k, MPI_INT, 4, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(minDisSumPivots2, M * k, MPI_INT, 4, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(maxDistanceSum2, M, MPI_DOUBLE, 4, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(minDistanceSum2, M, MPI_DOUBLE, 4, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
