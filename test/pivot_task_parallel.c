...
// Get C(n, k)
int combination_num(int n, int k){
    int i;
    long long result=1;
    for(i=n; i>=n-k+1; --i){
        result *= i;
...
void Combination(const int k, const int n, const int dim, const int M, const int size, const int rank,
        ...
        int i;
        int comb_sum = combination_num(n, k);
        int comb_local = (comb_sum + size - 1) / size;
        int comb_start = rank * comb_local; 
        int comb_num = (rank==size-1)? comb_sum-rank*comb_local : comb_local;
        ...
        for(i=0; i<comb_sum; ++i){
            if(i >= comb_start) break;
            nextCombination(n-1, k, pivots);
        }
        
        int kn;
        for(kn=0; kn<comb_num; ++kn){
...
void int_swap(int** array1, int** array2) {
    int* temp;
    temp = *array1;
    *array1 = *array2;
    *array2 = temp;
...
void double_swap(double** array1, double** array2) {
    double* temp;
    temp = *array1;
    *array1 = *array2;
    *array2 = temp;
...
int main(int argc, char* argv[]){
    ...
    // kn: Merge depth
    // size_n: valid size
    int kn = size / 2, size_n = size;
    while(kn > 0)
    {
        if(size_n % 2 == 0)
        {
            if(rank < 2*kn && rank >= kn){
                MPI_Send(maxDisSumPivots, M * k, MPI_INT, rank-kn, 0, MPI_COMM_WORLD);
                MPI_Send(minDisSumPivots, M * k, MPI_INT, rank-kn, 1, MPI_COMM_WORLD);
                MPI_Send(maxDistanceSum, M, MPI_DOUBLE, rank-kn, 2, MPI_COMM_WORLD);
                MPI_Send(minDistanceSum, M, MPI_DOUBLE, rank-kn, 3, MPI_COMM_WORLD);
            }
            else if(rank < kn){
                MPI_Recv(maxDisSumPivots1, M * k, MPI_INT, rank+kn, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(minDisSumPivots1, M * k, MPI_INT, rank+kn, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(maxDistanceSum1, M, MPI_DOUBLE, rank+kn, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(minDistanceSum1, M, MPI_DOUBLE, rank+kn, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                int_swap(&maxDisSumPivots, &maxDisSumPivots2);
                int_swap(&minDisSumPivots, &minDisSumPivots2);
                double_swap(&maxDistanceSum, &maxDistanceSum2);
                double_swap(&minDistanceSum, &minDistanceSum2);
        ...
        {
            if(rank == size_n - 1){
                MPI_Send(maxDisSumPivots, M * k, MPI_INT, rank-1, 5, MPI_COMM_WORLD);
                MPI_Send(minDisSumPivots, M * k, MPI_INT, rank-1, 6, MPI_COMM_WORLD);
                MPI_Send(maxDistanceSum, M, MPI_DOUBLE, rank-1, 7, MPI_COMM_WORLD);
                MPI_Send(minDistanceSum, M, MPI_DOUBLE, rank-1, 8, MPI_COMM_WORLD);
            }
            else if(rank == size_n - 2){
                MPI_Recv(maxDisSumPivots1, M * k, MPI_INT, rank+1, 5, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(minDisSumPivots1, M * k, MPI_INT, rank+1, 6, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(maxDistanceSum1, M, MPI_DOUBLE, rank+1, 7, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(minDistanceSum1, M, MPI_DOUBLE, rank+1, 8, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
